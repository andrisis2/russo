...
## Com'è fatta l'app
- `index.html` — PWA, legge `knowledge.json`. Home: saluto in russo (tocca per ascoltarlo), card **Ripasso del giorno** con anello di avanzamento e serie (`makeDailyHero`/`dailyProgress`), **I miei esercizi**, poi due schede: **Pratica** (Flashcard, Scrittura, Coniugazione, Frasi&ascolto, Componi la frase, Numeri e ore, Memory, Progressi, Vika) e **Teoria** (Vocabolario, Verbi, Dialoghi, Frasario, Cirillico, Regole, Grammatica, Frasi&grammatica). Sotto «I miei esercizi» c'è la **Parola del giorno** (`makeWotd`/`parolaDelGiorno`, scelta tra le parole non ancora imparate e fissata per la giornata). Le sezioni in `NUOVE_SEZIONI` hanno l'etichetta «Nuovo» finché non si aprono. SRS (Leitner) salvato in `localStorage`.
...
- **Progressi** (`runProgressi`): **traguardi** (`traguardi()`, 15 badge), stato del dizionario, mappa attività 18 settimane (chiave `russo_activity_v1`, alimentata da `grade()`), previsione ripassi a 7 giorni, parole ostiche.
...
- Chiavi in `localStorage`: `russo_srs_v1`, `russo_daily_v1`, `russo_esercizi_v1`, `russo_user_voci`, `russo_activity_v1`, `russo_settings_v1`, `russo_memory_best_v1`, `russo_extra_v1` (dialoghi ascoltati/recitati, numeri e frasi composte, parola del giorno), `russo_sezioni_viste`, `vika_*`.
...
- `knowledge.json` **versione 22** — **1631 voci** (ultimo id `v1631`), **190 frasi** (ultimo `f195`), **12 regole** quiz (ultimo `g012`), 120 `frasiGrammar` (id 100–219), 18 schede `riferimento` (`r018`), 216 `verbi` (`verb216`), **16 dialoghi** (id testuali: `bar`, `ristorante`… `museo`).
- **Sezione `riferimento`** (chiave a parte): 8 schede di consultazione "Regole grammaticali" (`r001`–`r008`: sei casi, pronomi declinati, possessivi, verbi al presente, passato, aspetto, plurale, numeri). Generata da `scripts/add_riferimento.py` (idempotente). NON sono drill: solo display + tap-per-ascoltare. Resa in `index.html` da `runReference()`. **Service worker: cache attuale `russo-v51`** (se modifichi `index.html` ricordati di bumpare la cache in `sw.js`, altrimenti i client di ritorno non vedono il nuovo file).
